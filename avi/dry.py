#eye detector ka code

import cv2
import dlib
from scipy.spatial import distance
import os
import threading
import playaudio
# import pygame


def calculate_EAR(eye):
    A = distance.euclidean(eye[1], eye[5])
    B = distance.euclidean(eye[2], eye[4])
    C = distance.euclidean(eye[0], eye[3])
    ear_aspect_ratio = (A+B)/(2.0*C)
    return ear_aspect_ratio

cap = cv2.VideoCapture(1)
# cap = cv2.VideoCapture('C:\\fest\\convluted-driveSync\\avi\\vid.mp4') # takin vid input updateinfffffo
hog_face_detector = dlib.get_frontal_face_detector()
dlib_facelandmark = dlib.shape_predictor("C:\\fest\\convluted-driveSync\\avi\\shape_predictor_68_face_landmarks.dat")


def openeye(frame):
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    faces = hog_face_detector(gray)

    for face in faces:
        face_landmarks = dlib_facelandmark(gray, face)
        leftEye = []
        rightEye = []

        for n in range(36,42):
            x = face_landmarks.part(n).x
            y = face_landmarks.part(n).y
            leftEye.append((x,y))
            next_point = n+1
            if n == 41:
                next_point = 36
            x2 = face_landmarks.part(next_point).x
            y2 = face_landmarks.part(next_point).y
            # cv2.line(frame,(x,y),(x2,y2),(0,255,0),1)

        for n in range(42,48):
            x = face_landmarks.part(n).x
            y = face_landmarks.part(n).y
            rightEye.append((x,y))
            next_point = n+1
            if n == 47:
                next_point = 42
            x2 = face_landmarks.part(next_point).x
            y2 = face_landmarks.part(next_point).y
            # cv2.line(frame,(x,y),(x2,y2),(0,255,0),1)

        left_ear = calculate_EAR(leftEye)
        right_ear = calculate_EAR(rightEye)

        EAR = (left_ear+right_ear)/2
        EAR = round(EAR,2)
        if EAR<0.26:
            # cv2.putText(frame,"eyes closed",(20,100),cv2.FONT_HERSHEY_SIMPLEX,3,(0,0,255),4)
            return 0
        else:
            return 1 



def warn():
    beep = 'C:\\fest\\convluted-driveSync\\avi\\beep.mp3'
    for i in range(0,3):
        playaudio.playaudio(beep)
    print("this ran")




def detect_eye():
    count = 0
    warning_thread_started = False

    while True:
        _, frame = cap.read()
        isOpened = openeye(frame)
        
        if isOpened == 0:
            count += 1
        else:
            count = 0
            if warning_thread_started:
                wt.join()
                warning_thread_started=False

        if count >= 20 and not warning_thread_started:
            warning_thread_started = True
            wt = threading.Thread(target=warn)
            wt.start()

        cv2.putText(frame, str(count), (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 4)
        cv2.imshow("eye", frame)
        
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

detect_eye()
# warn()